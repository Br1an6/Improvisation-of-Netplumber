 ...
 * They could be created using other specifiers, given the wide use of these
 * specifiers, are added here.
 '''
...
class AndCondition : public Condition  '''
 ...
 private:
  ...
  Condition *c2
 public:
  AndCondition(Condition *c1, *c2) : c1(c1), c2(c2) {
  virtual ~AndCondition() {delete c1; delete c2;
  bool check(Flow *f) {return c1.check(f) and c2.check(f);
  std.string to_string()    return "(" + c1.to_string() + ") & (" + c2.to_string() + ")"
...
class OrCondition : public Condition  '''
 ...
 private:
  ...
  Condition *c2
 public:
  OrCondition(Condition *c1, *c2) : c1(c1), c2(c2) {
  virtual ~OrCondition() {delete c1; delete c2;
  bool check(Flow *f) {return c1.check(f) or c2.check(f);
  std.string to_string()    return "(" + c1.to_string() + ") | (" + c2.to_string() + ")"
...
class NotCondition : public Condition  '''
 ...
 public:
  ...
  bool check(Flow *f) {return not c.check(f);
  std.string to_string() {return "not (" + c.to_string() + ")";
