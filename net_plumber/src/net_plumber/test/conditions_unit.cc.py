...
def test_table(self):  printf("\n")
  ...
  c.add_pathlet(new EndPathSpecifier())
  CPPUNIT_ASSERT(not c.check(*(flows.begin())))
  free_flow(flows)


def test_port_sequence(self):  printf("\n")
...
def test_path_length(self):  printf("\n")
...
def test_header(self):  printf("\n")
  ...
  HeaderCondition *hc1 = HeaderCondition(hc1_hs)
  HeaderCondition *hc2 = HeaderCondition(hc2_hs)

  CPPUNIT_ASSERT(hc1.check(f))
  CPPUNIT_ASSERT(not hc2.check(f))
...
def create_flow(self, ports, tables):  list<Flow*> *result = list<Flow*>()
  Flow *f, *prev
  SourceNode *s = SourceNode(NULL,1,0,hs_create(1),make_sorted_list(1,0))
  prev = (Flow *)malloc(sizeof *prev)
  prev.node = s
  prev.in_port = 0
  prev.p_flow = s.get_EOSFI()
  result.push_front(prev)
  for (i = 0; i< ports.size; i++)    RuleNode *r = RuleNode(NULL, 1, 0, tables.list[i],
...
def free_flow(self, flows):  list<Flow *>.iterator it
  for (it = flows.begin(); it != flows.end(); it++)    delete (*it).node
    free(*it)
