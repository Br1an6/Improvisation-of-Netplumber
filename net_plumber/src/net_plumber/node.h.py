...
def is_flow_looped(self, *f):

struct Pipeline  Node *node
  array_t *pipe_array
  uint32_t local_port
...
class Node protected:
  ...
  bool should_block_flow(Flow *f, out_port)
  void remove_flows()
  void remove_pipes()
 ...
 public:
  ...
  Node(void *plumber, length, node_id)
  virtual ~Node()
  NODE_TYPE get_type()
   ...
   * if erase is set, will be removed from @self node's source_flow and
   ...
   * source and sink flows going on that pipe.
   '''
  void remove_link_pipes(uint32_t local_port, remote_port)
  ...
  void count_src_flow(int &inc, &exc)

  '''
   * getting input/output state
   '''
  bool is_at_input_stage() { return self.is_input_layer;
  bool is_at_output_stage() { return self.is_output_layer;
