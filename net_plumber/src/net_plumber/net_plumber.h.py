...
struct Event  EVENT_TYPE type
  uint64_t id1;  #node id, id or source port.
  uint64_t id2;  # destination port.


def get_event_name(self, t):

namespace net_plumber
  class NetPlumber

  typedef void (*global_error_callback_t)(NetPlumber *N, *f, data)

  class NetPlumber   private:
    ...
    std.map<uint32_t, table_to_last_id
    ...
    uint64_t _add_rule(uint32_t table, index, group, gid,
                       List_t in_ports, out_ports,
                       array_t* match, *mask, rw)
   ...
   public:
    ...
    int get_length() { return self.length;
     ...
     * print_topology: prints current link connections.
     '''
    void add_link(uint32_t from_port, to_port)
    void remove_link(uint32_t from_port, to_port)
     ...
     * - add_rule_to_group: just like previous function, that accept a
     * rule id to group with. Note: for the first element in group, 0 as
     * group id. For later elements, index and table is ignored and it is
     * placed next to other element of the group in the same table.
     * remove_rule: removes a rule if exist
     '''
    uint64_t add_rule(uint32_t table, index, in_ports, out_ports,
                  array_t* match, *mask, rw)
    uint64_t add_rule_to_group(uint32_t table, index, in_ports,
                               List_t out_ports, match, *mask,
                               array_t* rw, group)
     ...
     * @ports: output ports
     '''
    uint64_t add_source(hs *hs_object, ports)
     ...
     *
     '''
    uint64_t add_source_probe(List_t ports, mode,
     ...
     * - get_pipe_stats: get pipe stats for node.
     '''
    void get_pipe_stats(uint64_t node_id, &fwd_pipeline, &bck_pipeline,
                    int &influence_on, &influenced_by)
    void get_source_flow_stats(uint64_t node_id, &inc, &exc)

    void save_dependency_graph(std.string file_name)

   private:
    void free_group_memory(uint32_t table, group)
    void free_rule_memory(RuleNode *r, remove_from_table=True)
