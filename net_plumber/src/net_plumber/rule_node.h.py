...
struct Influence  RuleNode *node
  std.list<struct Effect*>.iterator effect
  array_t *comm_arr
  List_t ports


struct Effect  RuleNode *node
  std.list<struct Influence*>.iterator influence


class RuleNode : public Node public:
   uint32_t table
  uint64_t group
  array_t *mask
  array_t *rewrite
   ...
   * constructor
   '''
  RuleNode(void *net_plumber, length, node_id, table,
  ...
  '''
   * generate the full string representing self rule.
   '''
   ...
   * - subtract_infuences_from_flows: call self function  when self rule is
   * added for the first time so that the influence of self flow on other nodes
