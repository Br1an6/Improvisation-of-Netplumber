...
enum PROBE_STATE  STOPPED = 0,
  STARTED,
  RUNNING,


enum PROBE_MODE  EXISTENTIAL,
  UNIVERSAL


enum PROBE_TRANSITION  UNKNOWN = 0,
  STARTED_TRUE,
  STARTED_FALSE,
  TRUE_TO_FALSE,
  FALSE_TO_TRUE,
  ...
  LESS_FALSE,  # in universal mode: less violating flow, still False
  LESS_TRUE    # in existential mode: less matching flow, still True


def probe_transition(self, t):

class SourceProbeNode

typedef void (*src_probe_callback_t)
    (void *caller, *p, *f, *data, PROBE_TRANSITION)

void default_probe_callback(void *caller, *p, *f,
                            void *data, t)
class SourceProbeNode : public Node protected:
  PROBE_STATE state
  PROBE_MODE mode
  Condition *filter
  Condition *test
 ...
 public:
  SourceProbeNode(void *n, length, node_id,
                  PROBE_MODE mode, ports,
                  Condition *filter, *condition,
                  src_probe_callback_t probe_callback, *callback_data)
  ...
  void absorb_src_flow(std.list<struct Flow*>.iterator s_flow, first)

  void update_check(Flow *f, action)
  void start_probe()
  void stop_probe()

  '''
   * get_condition_count: for existential mode, number of flows meeting
   * the condition. for universal case, number of flows violating a
   * condition.
   '''
  int get_condition_count() {return self.cond_count;

  PROBE_MODE get_mode() {return self.mode;
