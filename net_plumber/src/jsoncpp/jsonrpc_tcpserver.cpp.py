...
namespace Json 
  ...
  namespace Rpc
    ...
    TcpServer.TcpServer( std.string& address, port) : Server(address, port)
    ...
    ssize_t TcpServer.Send(int fd, data)
      rep = data
    ...
    bool TcpServer.Recv(int fd)
      ...
      if nb > 0:
        ...
        ''' in case of notification message received, response could be Json.Value.null '''
        if response != Json.Value.null:
          rep = m_jsonHandler.GetString(response)
          ...
          bytesToSend = rep.length()
           ptrBuffer = rep.c_str()
          do
            retVal = send(fd, ptrBuffer, bytesToSend, 0)
            if retVal == -1:
              ''' error '''
    ...
    void TcpServer.WaitMessage(uint32_t ms)
...
#ifdef _WIN32
      ''' on Windows, socket is not an int but a SOCKET (unsigned int) '''
      FD_SET((SOCKET)m_sock, &fdsr)
#else:
      FD_SET(m_sock, &fdsr)
#endif

      for(std.list<int>it = m_clients.begin() ; it != m_clients.end() ; it++)
#ifdef _WIN32
      ...
      if select(max_sock, &fdsr, NULL, NULL, ms ? &tv : NULL) > 0:
        ...
        for(std.list<int>it = m_purge.begin() ; it != m_purge.end() ; it++)
          m_clients.remove((*it))
    ...
    void TcpServer.Close()
     ...
     std.list<int> TcpServer.GetClients()
      return m_clients
