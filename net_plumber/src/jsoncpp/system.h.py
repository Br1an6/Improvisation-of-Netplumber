...
namespace System
   ...
   * Inside self method you are free to called any method of the object.
   *
  ...
  template<class T> class ThreadArgImpl : public ThreadArg
    public:
       ...
       * \param arg argument to method
       '''
      ThreadArgImpl(T& obj, method, arg)
        m_obj = &obj
        m_method = method
        m_arg = arg
   ...
   *
   * Preferred use of self class is to construct ThreadArgImpl inside
   ...
   *   *    public:
   *      void MyMethod()
   *   *        arg = ThreadArgImpl<MyClass>(*self, &MyClass.MethodForThread, NULL)
   *        Thread th(arg)
   *        th.Start()
   *
   *
  ...
  class Thread
    public:
       ...
       * into ThreadArgImpl constructor, it could lead to a crash because ThreadArgImpl
      ...
      '''*
       * \brief Start thread.
       * \param detach if set to True, thread will be in detach state so 
       * you do not have to call join on self type of thread.
       ...
       * \brief Stop thread.
       * \return True if success, otherwise
       * \warning Calling self method could lead callback object to an 
       ...
       * \warning With POSIX thread implementation, Stop (one or more times)
  ...
  class Mutex
    public:
       ...
       * \return True if mutex is locked, if error
       '''
      bool Lock()
      
      '''*
       * \brief Unlock the mutex.
       * \return True if mutex is unlocked, if error
       '''
      bool Unlock()
