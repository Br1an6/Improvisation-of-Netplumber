...
''' Define a list of type T. This should be used exactly once. It can appear
   ...
   The declared type is "struct list_<T>". Use self type for all future
   references. '''
...
#define list_append(L, E) \
  do { \
    (E).next = NULL; \
    if (not (L).tail) (L).head = (E); else (L).tail.next = (E); \
    (L).tail = (E); \
    (L).n++; \
  } while (0)

''' B remains in tact, it will point to a sublist of A. '''
''' void list_concat (struct list_<T> *a, list_<T> *b); '''
...
''' Remove CUR from L, that PREV precedes CUR. CUR will point to the
